function out = mpci_chronic_tuning_heatmaps(sessionPaths, alpha, opt)
% mpci_chronic_tuning_heatmaps plots task-variable tuning heatmaps for chronically
% tracked ROIs, divided by brain region
%
...
for i = 1:numel(sessionPaths)
    ...
    for f = 1:numel(fovs)
        ...
        if ~(isfile(pFile) && isfile(regFile))
            continue
        end
        ...
        if numel(regionIds) ~= n
            warning('Size mismatch (region vs P) in %s; skipping FOV.', fovPath);
            continue
        end
...
% -----------------------------
[G, ~, ~] = findgroups(Ruid.day, Ruid.clusterUID);
cnt = splitapply(@numel, Ruid.clusterUID, G);
if any(cnt > 1)
...
for j = 1:nCh
    uid = chUIDs(j);
    r = Rc.regionId(Rc.clusterUID == uid); % should be nDays long
...
minChronicPerRegion = 50;
nUIDsPerReg = [];

if ~isempty(Rc)
    % Count unique chronic UIDs per stable region
    [G, regVals] = findgroups(double(Rc.stableRegionId));
    nUIDsPerReg = splitapply(@(u) numel(unique(u)), Rc.clusterUID, G);

    keepReg = nUIDsPerReg >= minChronicPerRegion;
    keepRegionIds = regVals(keepReg);

    % Filter Rc to keep only those regions
...
for ti = 1:nTests
    ...
    if nRegions > 0
        ...
        ax2.YTickLabel = regionLabels + " (" + string(nUIDsPerReg) + ")";
    else
        ax2.YTickLabel = {};
    end
